...
class CustomerRequirementController extends Controller
{
    ...
    public function view($id, $crrNumber)
    {
        ...
        $price_currencies = PriceCurrency::all();
        $nature_requests = NatureRequest::all();
        $rnd_personnel = User::whereHas('role', function($roleQuery) {
                $roleQuery->whereIn('type', ['RND', 'QCD-WHI', 'QCD-MRDC', 'QCD-CCC', 'QCD-PBI']);
            })
    ...
    public function CancelCrr(Request $request, $id)
    {
        try {
            ...
            Alert::error('Error! Failed to cancel the Customer Requirement');
            return back();
        }
        
        Alert::success('Successfully Cancelled Customer Requirement')->persistent('Dismiss');
        return redirect()->route('customer_requirement.index');
    }
