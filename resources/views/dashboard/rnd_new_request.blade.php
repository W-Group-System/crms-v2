                        ...
                        @if(count($paginatedResults) > 0)
                            @foreach ($paginatedResults as $transaction)
                                ...
                                <tr>
                                    <td>
                                        @if(str_contains($transaction->CrrNumber, 'CRR'))
                                        <a href="{{url('view_customer_requirement/'.$id.'/'.$transaction_number.'/?origin=new_request')}}">
                                        @elseif(str_contains($transaction->RpeNumber, 'RPE'))
                                        <a href="{{url('product_evaluation/view/'.$id.'/'.$transaction_number.'/?origin=new_request')}}">
                                        @elseif(str_contains($transaction->SrfNumber, 'SRF'))
                                        <a href="{{url('samplerequest/view/'.$id.'/'.$transaction_number.'/?origin=new_request')}}">
                                        @endif
                                            {{$transaction_number}}
                                        </a>
                                    </td>
